...
def inference_then_eval(
    ...
    if save_infer_result:
        save_path = os.path.join(experiment_dir,'Nest_infer_result_{}.pkl'.format(save_tag))
        with open(save_path,'wb') as f:
            pickle.dump(infer_result_for_save,f)
        logger.info("infer_result saved at {}".format(save_path))

    if save_relation_json:
        save_path = os.path.join(experiment_dir,'Nest_predict_relations_{}.json'.format(save_tag))
